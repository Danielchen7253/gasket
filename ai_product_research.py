...
def normalize_research(research: dict[str, Any], brand: str, model: str) -> dict[str, Any]:
    ...
    if len(gaskets) == 1 and len(positions) > 1:
        only = dict(gaskets[0])
        only_key = clean_door_key(only.get("door_position"), 1)
        only_label = (only.get("door_position_display") or "").lower()
        looks_generic = only_key.startswith("door_") or not only_label or only_label in {"door", "gasket"}
        if looks_generic:
            expanded = []
            base_evidence = only.get("evidence_summary") or "Single generic gasket candidate split across confirmed door positions."
            for index, position in enumerate(positions, start=1):
                item = dict(only)
                item["door_index"] = index
                item["door_position"] = position["key"]
                item["door_position_display"] = position["label"]
                item["gasket_name"] = item.get("gasket_name") or f"{position['label']} gasket"
                item["evidence_summary"] = base_evidence
                item["needs_customer_confirmation"] = True
                item["customer_confirmation_note"] = item.get("customer_confirmation_note") or "Confirm door side and dimensions before production."
                item["confidence_score"] = min(clamp_score(item.get("confidence_score"), 70), 80)
