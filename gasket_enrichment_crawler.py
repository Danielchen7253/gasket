...
PRICE_RE = re.compile(r"\$\s*(?P<price>\d{2,4}(?:,\d{3})*(?:\.\d{2})?)")
BAD_PART_NUMBERS = {
    "REPLACE",
    "REPLACES",
    "NERS",
...
def normalize_dimension_pair(width: float | None, height: float | None) -> tuple[float | None, float | None]:
    if not width or not height:
        return width, height
    # Some search result pages expose metric dimensions such as 405x570.
    # Store inches in numeric fields so quoting and comparison stay consistent.
...
def is_valid_gasket_detail(detail: dict) -> bool:
    ...
    if detail.get("universal_part_number") and not canonical_part_number(detail["universal_part_number"]):
        detail["universal_part_number"] = None
    return bool(
...
def extract_detail(source_name: str, url: str, html: str, brand: str = "", model: str = "") -> dict | None:
    ...
    if dim_match:
        ...
        width_in, height_in = normalize_dimension_pair(width_in, height_in)
...
def get_pending_models(client: httpx.Client, limit: int = 50) -> list[dict]:
    ...
    if not ids:
        direct_endpoint = (
            ...
            "&data_status=in.(pending,missing,not_found)"
            f"&limit={limit}"
        )
...
def upsert_application(client: httpx.Client, detail: dict, part_id: int | None) -> None:
    ...
    if response.status_code >= 400:
        # The database uses an expression index for null door positions, so a direct
        # upsert can fail. A plain insert is safe enough; duplicates are harmless here.
        response = client.post(
            f"{SUPABASE_URL.rstrip('/')}/rest/v1/{APPLICATION_TABLE}",
            headers=supabase_headers("return=minimal"),
            json=row,
...
def rank_details(details: list[dict]) -> list[dict]:
    ...
    dim_counts: dict[str, set[str]] = {}
    part_counts: dict[str, set[str]] = {}
    for detail in details:
        source = detail.get("source_name") or detail.get("source_url") or "unknown"
        dim = dimension_key(detail)
        part = canonical_part_number(detail.get("gasket_part_number"))
        if dim:
            dim_counts.setdefault(dim, set()).add(source)
        if part:
            part_counts.setdefault(part, set()).add(source)
    ...
    for detail in details:
        ...
        evidence_count = 1
        if dim:
            evidence_count = max(evidence_count, len(dim_counts.get(dim, set())))
            adjusted += min(18, (len(dim_counts.get(dim, set())) - 1) * 9)
        if part:
            evidence_count = max(evidence_count, len(part_counts.get(part, set())))
            adjusted += min(12, (len(part_counts.get(part, set())) - 1) * 6)
        ...
        copy = dict(detail)
        copy["rank_score"] = round(min(100, adjusted), 2)
...
def main() -> None:
    ...
    with httpx.Client(timeout=60) as client:
        ...
        models = get_pending_models(client, limit=limit - refreshed)
