...
def brand_adjacent_models(brand: str, *values: str) -> set[str]:
    ...
    for alias in brand_aliases(brand):
        ...
        for token in tokens[:6]:
            ...
            if token_u in PRODUCT_WORDS:
                continue
            if not re.search(r"\d", token_u):
                continue
...
def lifecycle_from_text(text: str) -> str:
    ...
    if any(token in haystack for token in discontinued_tokens):
        return "discontinued"
    if any(token in haystack for token in active_tokens):
        return "active"
    return "unknown"
...
def search_bing_web(client: httpx.Client, query: str, limit: int) -> list[SearchResult]:
    ...
    if results:
        return results
    return search_bing_images(client, query, limit)
...
def extract_candidates(result: SearchResult, final_url: str, html: str) -> list[dict]:
    ...
    title_haystack = f"{page_title} {result.title}".lower()
    ...
    models = brand_adjacent_models(brand, page_title, result.title)
    if not models:
        models = brand_adjacent_models(brand, result.snippet)
    ...
    for model in sorted(models)[:8]:
        ...
        if brand:
            confidence += 30
        if model and normalized(model) in normalized(" ".join([page_title, result.title])):
            confidence += 35
        elif model and normalized(model) in normalized(result.snippet):
            confidence += 20
        if product_type:
            confidence += 10
        if image_url:
        ...
        if any(token in final_url.lower() for token in ["product", "products", "spec", "manual"]):
            confidence += 5
        confidence += source_weight(kind)
        if any(token in normalized(model) for token in ["PDF", "HTML"]):
            confidence -= 15
        if any(token in title_haystack for token in PART_PAGE_TOKENS) and not (spec_sheet or manual):
            confidence -= 12
        ...
        rows.append(
            {
                ...
                "review_status": "auto_ready" if confidence >= PROMOTE_SCORE else "pending",
                "evidence": {
...
def upsert_discovery(client: httpx.Client, row: dict) -> dict:
    endpoint = (
        ...
        "?on_conflict=normalized_brand,normalized_model,source_url"
    )
    row = dict(row)
...
def update_discovery_review(client: httpx.Client, discovery: dict) -> tuple[str, dict]:
    ...
    response = client.patch(
        ...
        json={
            "confidence_score": aggregate["aggregate_score"],
    ...
    enriched["confidence_score"] = aggregate["aggregate_score"]
...
def promote_discovery(client: httpx.Client, discovery: dict) -> int | None:
    ...
    existing = get_existing_product(
        client,
        discovery["discovered_brand"],
        discovery["discovered_model"],
    )
    ...
    if existing:
        ...
        response = client.patch(
            endpoint,
            headers=supabase_headers("return=minimal"),
            json={key: value for key, value in product_row.items() if value is not None},
        )
        response.raise_for_status()
        return existing["id"]
...
def update_discovery_promotion(client: httpx.Client, discovery_id: int, product_id: int | None) -> None:
    ...
    response = client.patch(
        endpoint,
        headers=supabase_headers("return=minimal"),
        json={"promoted_product_id": product_id, "review_status": status},
    )
    response.raise_for_status()
        ...
        for query in build_queries(query_limit):
            ...
            for result in search_bing_web(client, query, results_per_query):
                ...
                for row in extract_candidates(result, final_url, html):
                    ...
                    saved_count += 1
                    _, saved = update_discovery_review(client, saved)
