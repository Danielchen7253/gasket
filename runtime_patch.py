...
<div>Status</div><div>{esc(product.get('lifecycle_status') or 'unknown')}</div>
        ...
        for index, item in enumerate(quote_items, start=1):
            ...
            image_html = f"<img src='{esc(image)}' alt='Gasket product image'>" if image else "<span>Product image loading</span>"
...
.gasket-option{{display:grid;grid-template-columns:28px 112px minmax(220px,1fr) 128px;gap:12px;align-items:center;border:1px solid #dbe2ea;border-radius:8px;padding:10px 14px;margin:0;background:#fff}}
...
.gasket-copy{{width:100%;max-width:360px}}
