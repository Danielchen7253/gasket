...
def upsert_candidate(client: httpx.Client, product: dict, candidate: dict, probe_dimensions: bool = True) -> dict:
    if not candidate.get("image_width") or not candidate.get("image_height"):
        width, height = dimensions_from_url(candidate["image_url"])
        if probe_dimensions and (not width or not height):
...
def quick_promote_product_image(client: httpx.Client, product: dict, limit: int = 6) -> bool:
    ...
    saved = [
        upsert_candidate(client, product, row, probe_dimensions=False)
